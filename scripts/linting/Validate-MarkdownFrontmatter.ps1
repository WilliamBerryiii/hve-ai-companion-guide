...
function Get-ValidationConfig {
    <#
    .SYNOPSIS
    Loads validation configuration from validation-config.json.
    
    .DESCRIPTION
    Loads the centralized validation configuration that maps file patterns to schema rules.
    Returns null if config file is missing or invalid.
    
    .OUTPUTS
    PSCustomObject containing version and rules array, or null if loading fails.
    #>
    
    $configPath = Join-Path $PSScriptRoot "..\schemas\validation-config.json"
    
    if (-not (Test-Path $configPath)) {
        Write-Warning "Validation config not found: $configPath"
        return $null
    }
    
    try {
        $config = Get-Content -Path $configPath -Raw | ConvertFrom-Json
        Write-Verbose "Loaded validation config version $($config.version)"
        return $config
    }
    catch {
        Write-Warning "Failed to parse validation config: $($_.Exception.Message)"
        return $null
    }
}

function Get-ContentTypeRule {
    <#
    .SYNOPSIS
    Matches a file path against validation config patterns to determine content type.
    
    .DESCRIPTION
    Takes a file path and validation config, normalizes the path, calculates relative path
    from repository root, and matches against config patterns in order. Returns the first
    matching rule or null if no match found.
    
    .PARAMETER FilePath
    Absolute path to the markdown file to match.
    
    .PARAMETER Config
    Validation config object from Get-ValidationConfig.
    
    .OUTPUTS
    PSCustomObject containing the matched rule (name, pattern, schema, requiredFields, etc.)
    ...
    if (-not $repoRoot) {
        Write-Warning "Could not determine repository root for: $FilePath"
        return $null
    }
    ...
    foreach ($rule in $Config.rules) {
        # Handle location constraint for root-level files
        if ($rule.location -eq 'root') {
            $fileName = Split-Path -Leaf $FilePath
            if ($fileName -match $rule.pattern) {
                $fileDir = Split-Path -Parent $FilePath
                $fileDir = $fileDir -replace '\\', '/'
                if ($fileDir -eq $repoRoot) {
                    Write-Verbose "Matched rule: $($rule.name) (root-level)"
                    return $rule
                }
            }
            continue
        ...
        if ($rule.excludePattern -and $relativePath -match $rule.excludePattern) {
            Write-Verbose "Excluded by rule: $($rule.name)"
            continue
        }
    ...
    Write-Verbose "No matching rule found for: $relativePath"
    return $null
}
...
function Get-SchemaDefinition {
    <#
    .SYNOPSIS
    Loads JSON Schema definition file for documentation reference.
    
    .DESCRIPTION
    Loads the JSON Schema file for a content type. Used for documentation purposes
    and to understand schema structure. Actual validation uses native PowerShell.
    
    .PARAMETER SchemaName
    Name of the schema file (e.g., "chapter-content.schema.json").
    
    .OUTPUTS
    PSCustomObject containing the parsed JSON Schema, or null if loading fails.
    #>
    ...
    $schemaPath = Join-Path $PSScriptRoot "..\schemas\$SchemaName"
    ...
    try {
        ...
        Write-Verbose "Loaded schema: $SchemaName"
        return $schema
    }
    catch {
        Write-Warning "Failed to parse schema $SchemaName : $($_.Exception.Message)"
        return $null
    }
}
...
function Test-FrontmatterValidation {
    ...
    # Load validation configuration for schema-based validation
    $validationConfig = Get-ValidationConfig
    if ($validationConfig) {
        Write-Verbose "Loaded validation config version $($validationConfig.version) with $($validationConfig.rules.Count) rules"
    }
    else {
        Write-Verbose "Validation config not available, using legacy validation only"
    }
    ...
    foreach ($file in $markdownFiles) {
        ...
        # Check if file should be excluded based on validation config
        $relativePath = $file.FullName.Replace($repoRoot, '').TrimStart('\', '/').Replace('\', '/')
        if ($validationConfig -and $validationConfig.excludePatterns) {
            $shouldExclude = $false
            foreach ($pattern in $validationConfig.excludePatterns) {
                if ($relativePath -like $pattern) {
                    Write-Verbose "Excluding file based on config pattern: $relativePath"
                    $shouldExclude = $true
                    break
                }
            }
        ...
        try {
            ...
            if ($frontmatter) {
                ...
                $rule = $null
                if ($validationConfig) {
                    $rule = Get-ContentTypeRule -FilePath $file.FullName -Config $validationConfig
                }

                if ($rule) {
                    # Schema-based validation (new approach)
                    Write-Verbose "Applying schema rule: $($rule.name)"

                    # Validate required fields
                    foreach ($field in $rule.requiredFields) {
                        if (-not $frontmatter.Frontmatter.ContainsKey($field)) {
                            $errorMsg = "Missing required field '$field' (rule: $($rule.name)) in: $($file.FullName)"
                            $errors += $errorMsg
                            [void]$filesWithErrors.Add($file.FullName)
                            Write-GitHubAnnotation -Type 'error' -Message "Missing required field '$field' (rule: $($rule.name))" -File $file.FullName
                        }
                    }

                    # Validate suggested fields (warnings only)
                    if ($rule.suggestedFields) {
                        foreach ($field in $rule.suggestedFields) {
                            if (-not $frontmatter.Frontmatter.ContainsKey($field)) {
                                $warningMsg = "Suggested field '$field' missing (rule: $($rule.name)) in: $($file.FullName)"
                                $warnings += $warningMsg
                                [void]$filesWithWarnings.Add($file.FullName)
                                Write-GitHubAnnotation -Type 'warning' -Message "Suggested field '$field' missing (rule: $($rule.name))" -File $file.FullName
                            }
                        }
                    }

                    # Validate date format
                    if ($rule.datePattern -and $rule.dateField) {
                        if ($frontmatter.Frontmatter.ContainsKey($rule.dateField)) {
                            $date = $frontmatter.Frontmatter[$rule.dateField]
                            if ($date -notmatch $rule.datePattern) {
                                $warningMsg = "Invalid date format in $($rule.dateField) (rule: $($rule.name)). Expected pattern: $($rule.datePattern), got: $date in: $($file.FullName)"
                                $warnings += $warningMsg
                                [void]$filesWithWarnings.Add($file.FullName)
                                Write-GitHubAnnotation -Type 'warning' -Message "Invalid date format in $($rule.dateField): Expected pattern $($rule.datePattern), got: $date" -File $file.FullName
                            }
                        }
                    }
                    ...
                    if ($rule.titlePattern) {
                        if ($frontmatter.Frontmatter.ContainsKey('title')) {
                            $title = $frontmatter.Frontmatter['title']
                            if ($title -notmatch $rule.titlePattern) {
                                $warningMsg = "Title format does not match expected pattern (rule: $($rule.name)) in: $($file.FullName)"
                                $warnings += $warningMsg
                                [void]$filesWithWarnings.Add($file.FullName)
                                Write-GitHubAnnotation -Type 'warning' -Message "Title format does not match expected pattern (rule: $($rule.name))" -File $file.FullName
                            }
                        }
                    }

                    # Validate prohibited content
                    if ($rule.prohibitedContent) {
                        foreach ($pattern in $rule.prohibitedContent) {
                            $content = $frontmatter.Content
                            if ($content -match $pattern) {
                                $errorMsg = "Prohibited content pattern found (rule: $($rule.name)) in: $($file.FullName)"
                                $errors += $errorMsg
                                [void]$filesWithErrors.Add($file.FullName)
                                Write-GitHubAnnotation -Type 'error' -Message "Prohibited content pattern found (rule: $($rule.name))" -File $file.FullName
                            }
                        }
                    }

                    # Validate footer requirement
                    if (-not $SkipFooterValidation -and $rule.footerRequired -and $frontmatter.Content) {
                        $hasFooter = Test-MarkdownFooter -Content $frontmatter.Content
                        
                        if (-not $hasFooter) {
                            $footerMessage = "Missing standard Copilot footer (rule: $($rule.name)) in: $($file.FullName)"
                            $errors += $footerMessage
                            [void]$filesWithErrors.Add($file.FullName)
                            Write-GitHubAnnotation -Type 'error' -Message "Missing standard Copilot footer (rule: $($rule.name))" -File $file.FullName
                        }
                    }

                    # Universal validations still apply
                    ...
                    # Fallback to legacy validation when no schema rule matches
                    Write-Verbose "No schema rule found, using legacy validation for: $($file.FullName)"
                ...
                }  # End of legacy validation else block
            }
            else {
