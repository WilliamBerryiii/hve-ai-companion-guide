...
* **Tests**: ❌ 8% code coverage (mostly outdated)
* **Linting**: ⚠️ ESLint installed but not enforced (800+ warnings)
* **Documentation**: ❌ Outdated README, no inline comments, no ADRs
* **Source Control**: ❌ "fix", "update", "WIP" commit messages
...
* ✅ Notifications work in local dev (single server)
* ❌ Notifications fail in staging (multi-server, load balanced)
* ❌ Order creation fails if Socket.io server down
* ❌ Memory leaks in disconnection handling
...
* **Tests**: ✅ 35% coverage (critical paths covered)
* **Linting**: ✅ Enforced with pre-commit hooks
* **Documentation**: ✅ README + ADRs + inline comments
* **Source Control**: ✅ Conventional commits + PR reviews
...
1. **AI amplifies existing quality** (good or bad)
   * Poor fundamentals → AI suggestions compound problems
...
4. **Documentation teaches AI your patterns**
   * README + ADRs → AI suggests architecturally consistent code
   * No docs → AI guesses (poorly)
